...
// Exhaust smoothing (90% old / 10% new)
double smoothExhaustF(double raw) {
    static double smoothed = raw;
    smoothed = (smoothed * 0.90) + (raw * 0.10);
    return smoothed;
}

// Burn logic smoothing (70% old / 30% new)
int smoothBurnOutput(int raw) {
    static float smoothed = raw;
    smoothed = (smoothed * 0.70f) + (raw * 0.30f);
    return (int)(smoothed + 0.5f);
}

// Fan smoothing (85% old / 15% new)
int smoothFanPercent(int raw) {
    static float smoothed = raw;
...
void loop() {
    ...
    // --------------------------------------------------------
    // Smooth burn logic output
    // --------------------------------------------------------
    int smoothBurn = smoothBurnOutput(rawFan);

    // --------------------------------------------------------
    // Smooth fan % BEFORE applying fan control
    // --------------------------------------------------------
    int smoothFan = smoothFanPercent(smoothBurn);

    // --------------------------------------------------------
    // Apply BOOST, clamp, deadzone, PWM
    // --------------------------------------------------------
    fancontrol_updateBoost();
    int finalFan = fancontrol_apply(smoothFan);
