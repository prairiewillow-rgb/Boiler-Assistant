...
// Simple UI refresh throttle for HOME only
...
static void showBootScreen() {
    ...
    delay(300);
    lcd.setCursor(0,1); lcd.print("   v1.3 (OFFLINE)   ");
    delay(300);
...
// ============================================================
//  Home Screen (no throttle here, just pure render)
// ============================================================
...
static void ui_showClampMenu() {
    ...
    snprintf(l4,21,"4:Deadzone Fan: %s", deadzoneFanMode ? "ON" : "OFF");
    lcd4(l1,l2,l3,l4);
}
...
void ui_showScreen(int state, double exhaustF, int fanPercent) {
    ...
    // Throttle HOME screen only (350ms), others always draw
    if (state == UI_HOME) {
        unsigned long now = millis();
        if (now - lastUIRefresh < 350) {
            return;
        }
        lastUIRefresh = now;
    }
...
void ui_handleKey(char k, double exhaustF, int fanPercent) {
    ...
    // uiNeedRedraw is still available globally if you use it elsewhere,
    // but we don't rely on it for throttling anymore.
    uiNeedRedraw = true;
