/*
 * ============================================================
 *  Boiler Assistant – Burn Engine (v2.1)
 *  ------------------------------------------------------------
 *  File: BurnEngine.cpp
 ...
 *
 *  v2.1 Critical Fix:
 *      - HOLD phase now outputs 0% demand when exhaust >= setpoint
 *        (prevents runaway temps and allows fan to turn OFF)
 *
 * ============================================================
 */
...
int burnengine_compute(double smoothExh) {
    ...
    if (burnState == BURN_HOLD) {
        ...
        // 🔥 CRITICAL FIX:
        // If exhaust >= setpoint → fan OFF
        if (error <= 0) {
            return 0;
        }

        // Zone 1: just below setpoint
        if (error <= 5) {
